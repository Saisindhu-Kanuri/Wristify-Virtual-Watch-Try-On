...
async function loadHandDetectionModel() {
    ...
    try {
        // Check if tf.js and handpose are available
        if (!window.tf || typeof tf.ready !== 'function') {
            throw new Error("TensorFlow.js is not loaded or incomplete");
        }
        if (!window.handpose) {
            throw new Error("Handpose model is not available");
        }

        await tf.ready(); // Ensure tf is fully initialized
        ...
        try {
            ...
            console.warn("Custom model config failed. Falling back to default...");
            model = await handpose.load(); // fallback
        }
        ...
        showError(`❌ Failed to load hand detection model: ${error.message}. Try refreshing or use Google Chrome.`);
...
function determineHandType(landmarks) {
    ...
    // For mirrored webcam: if pinky is to the left of index, it's the right hand
...
function determinePalmDirection(landmarks) {
    ...
    // Cross product to estimate palm normal
    const palmNormal = {
    ...
    // Z-depth difference between wrist and fingers
    const zDifference = fingerBasesZ - wristZ;
    const zThreshold = 0.015;

    // Estimate from Z-depth
    const palmDirection = zDifference < -zThreshold ? 'down' : 'up';

    // Normalize palm normal vector (optional but useful)
    const mag = Math.sqrt(palmNormal.x ** 2 + palmNormal.y ** 2 + palmNormal.z ** 2);
    const normalizedZ = mag !== 0 ? palmNormal.z / mag : 0;

    // Estimate from palm normal
    const normalBasedDirection = normalizedZ > 0 ? 'up' : 'down';

    // Combine both methods for a more confident result
    return palmDirection === normalBasedDirection
        ? palmDirection
...
async function detectWrist() {
    if (!videoElement || videoElement.readyState !== videoElement.HAVE_ENOUGH_DATA || !model) return null;
    ...
    try {
        ...
        const confidenceScore = predictions[0].handInViewConfidence;

        // Detect hand type (left/right) and palm direction (up/down)
        const detectedHandType = determineHandType(landmarks);
        const palmDirection = determinePalmDirection(landmarks);

        // Check if shown hand matches selected hand
        ...
        // Estimate wrist width
        const wristWidthPx = Math.hypot(
            indexMCP[0] - pinkyMCP[0],
            indexMCP[1] - pinkyMCP[1]
        ) * 1.2;

        // Calculate orientation angle (based on wrist to middle finger)
        const wristToMiddle = {
        ...
        // Offset for watch placement (slightly up or down on wrist)
        ...
        // Final watch placement coordinates
        ...
        // Draw landmarks and wrist overlay for visual debugging
        drawLandmarks(landmarks);
        drawWristRegion(wristX, wristY, regionWidth, regionHeight, angle, palmDirection);

        // Prepare wrist placement info for overlay
        ...
        // Apply smoothing using position history
        lastFewPositions.push(currentWristPosition);
        if (lastFewPositions.length > POSITION_HISTORY_LENGTH) {
            lastFewPositions.shift();
        }

        const smoothedPosition = getSmoothedPosition(currentWristPosition);
        lastWristPosition = smoothedPosition;
